...
const PeerJSRoomVideoCall = () => {
...

	{
		if (!isInRoom) {
			return (
				<>
					<div className="fixed inset-0 bg-gradient-to-br from-green-100 to-emerald-200 flex items-center justify-center overflow-hidden">
						<div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-7xl mx-auto h-auto max-h-full overflow-y-auto">
							...
							<div className="text-center mb-8">
								<div className="inline-flex items-center justify-center gap-4 mb-4">
									{/* ใช้รูปโลโก้แทนไอคอน เพื่อให้ดูเป็นแบรนด์มากขึ้น */}
									<img
							...
							<div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-6">
								...
								<div className="flex flex-col">
									<label className="block text-2xl font-medium text-gray-700 mb-2 text-left">
										บทบาทของคุณ
									</label>
									...
									<div className="relative flex-1">
										<button
											...
											className="w-full h-full min-h-14 px-4 py-3 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition flex items-center justify-between text-left"
										>
										...
										{/* Dropdown Menu */}
										{isRoleDropdownOpen && (
											<div className="absolute z-10 w-full mt-2 bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden text-2xl">
												<button
													onClick={() => {
														setUserRole('client');
								...
								<div className="flex flex-col">
									...
									{selectedLocation ? (
										<div className="h-full min-h-14 p-4 bg-emerald-50 border border-emerald-200 rounded-lg flex items-center justify-between">
											<div className="flex items-center gap-3 text-3xl">
										...
										<div className="h-full min-h-14 p-4 bg-yellow-50 border border-yellow-200 rounded-lg flex flex-col items-center justify-center text-center">
											<p className="text-yellow-800 font-medium mb-3">ยังไม่ได้เลือกสถานที่</p>
											<button
												onClick={() => setShowLocationSettings(true)}
												className="px-6 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg text-sm font-medium transition"
											>
												ตั้งค่าสถานที่
											</button>
										</div>
											...
											<div>
												...
												<div className="flex gap-2">
													<input
														...
														className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none uppercase font-mono"
													/>
													<button onClick={generateRoomId} className="px-4 py-3 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 rounded-lg font-medium">
														สร้าง
													</button>
												</div>
											</div>
													...
													<div className="space-y-3 max-h-64 overflow-y-auto">
														{availableRooms.map((room) => (
															<button
																...
																className="w-full p-4 bg-white hover:bg-emerald-50 border border-gray-300 rounded-lg text-left transition shadow-sm flex items-center justify-between group"
															>
																<div>
																	<p className="font-semibold text-gray-900 group-hover:text-emerald-700">{room.roomId}</p>
