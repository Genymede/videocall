...
const PeerJSRoomVideoCall = () => {
    ...
    useEffect(() => {
		...
		// บังคับอัพเดท local video ทุกครั้งที่มี stream
		useEffect(() => {
		...
		// เพิ่มตรงนี้ หลังจาก const localVideoRef = useRef(null);
		useEffect(() => {
			if (localVideoRef.current && localStreamRef.current) {
				localVideoRef.current.srcObject = localStreamRef.current;
			}
		}, [isInRoom]); // ทุกครั้งที่เข้าห้อง ให้แน่ใจว่า local video มี stream
    ...
    const initializePeer = () => {
        ...
        peer.on('disconnected', () => {
            console.log('⚠️ Disconnected from PeerJS server, attempting reconnect...');
            peer.reconnect();
        });
    ...
    const joinRoom = async () => {
        ...
        try {
            ...
            const roomKey = `peerjs_room_${roomId}`;
            const hostPeerId = localStorage.getItem(roomKey);

            if (!hostPeerId || hostPeerId === myPeerId) {
                console.log('🏠 Creating new room as host');
                localStorage.setItem(roomKey, myPeerId);
    ...
    const callPeer = async (peerId, peerName) => {
        if (callsRef.current[peerId]) {
            console.log('⏭️ Already calling', peerId);
            return;
        }
    ...
    //       label.className = 'absolute bottom-4 left-4 bg-black bg-opacity-50 px-3 py-1 rounded-lg';
    //       label.innerHTML = `<span class="text-white text-sm">${metadata?.name || 'User'}</span>`;
    ...
    //         videoElement.autoplay = true;
    //         videoElement.playsInline = true;
    //         videoElement.muted = false; // ไม่ mute วิดีโอของคนอื่น
    //         videoElement.className = 'w-full h-full object-cover';
    ...
    //             console.log('▶️ Video playing for:', peerId);
    //         };
    ...
    const leaveRoom = (notifyOthers = true) => {
        if (notifyOthers) {
            if (isHost) {
                console.log('👑 Host leaving, closing room');
                localStorage.removeItem(`peerjs_room_${roomId}`);
                    ...
                    <div className="mt-6 space-y-3">
                        ...
                        <div className="p-4 bg-purple-50 rounded-lg border border-purple-200">
                            <p className="text-sm text-purple-800">
                                <strong>💡 คุณสมบัติ:</strong> P2P Data Connection สำหรับ sync ผู้ใช้อัตโนมัติ
                            </p>
                        </div>
                    </div>
                </div>
                ...
                {isHost && participants.length > 1 && (
                    <div className="mt-4 text-center">
                        <p className="text-yellow-400 text-sm">
                            ⚠️ คุณเป็น Host - เมื่อกด "ปิดห้อง" จะปิดการสนทนาสำหรับทุกคน
                        </p>
                    </div>
                )}
            </div>
